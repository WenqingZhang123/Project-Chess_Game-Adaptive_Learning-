...
class Agent:
    ...
    def select_action(self, state, allowed_actions, testing=False):
        # print(state.shape)  # 打印state的形状
        # if state.numel() == 0 or allowed_actions.numel() == 0:  # 检查state和allowed_a是否为空
        # if state.numel() == 0 or allowed_actions.size == 0:  # 检查state和allowed_a是否为空
        #     return -1  # 返回一个特殊的动作，表示游戏结束
    ...
    def update(self, state, action, reward, next_state, done, next_action, allowed_actions_next):
        ...
        # q_target = reward + (self.gamma * q_values_next[next_action]) * (
        #             1.0 - done)  # Use the Q value of the next state-action pair
